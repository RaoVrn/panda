...
export function ViewportObserver({ lessonId }: { lessonId: string }) {
  ...
  // Scroll → scroll % + the block currently in the viewport.
  useEffect(() => {
    if (!structure) return;
    ...
    const update = () => {
      ...
      for (const el of Array.from(blocks)) {
        const rel = el.getBoundingClientRect().top - scrollerRect.top;
        if (rel <= threshold) current = el.dataset.blockId;
        else break;
      }

      const block = current ? structure.blockById(current) : undefined;
      reportRef.current({
        scrollPercent: pct,
        ...
        currentSection: current
          ? structure.headingsOf(current).heading
          : structure.sectionAtProgress(pct),
        // A scroll changes the referent of "this"; require a fresh selection.
