...
export interface LearningWorkspaceProps {
  children: ReactNode;
  /** Set false on the dashboard, which stays a focused learning surface. */
  showAi?: boolean;
}
 ...
 */
export function LearningWorkspace({ children, showAi = true }: LearningWorkspaceProps) {
  const isDesktop = useIsDesktop();
  const { pathname } = useLocation();
  // The currently open lesson slug (if on a lesson page), for sidebar context.
  const currentSlug = pathname.startsWith("/lesson/") ? pathname.split("/")[2] : undefined;
  // The currently open module, derived from the URL (module page or a lesson).
  const currentModuleId = (() => {
    if (pathname.startsWith("/module/")) return pathname.split("/")[2];
    if (currentSlug) return moduleOfLesson(getLessonBySlug(currentSlug)?.id ?? "")?.id;
    return undefined;
  })();
  ...
  const aiActive = showAi && (isDesktop ? aiOpen : mobileAi);
  ...
  const handleContentClick = () => {
    if (showAi && isDesktop && aiOpen) setAiOpen(false);
  };
        ...
        {mobileSidebar && !isDesktop && (
          <>
            ...
            <motion.aside
              ...
              aria-label="Course navigation"
            >
              <CourseSidebar onClose={() => setMobileSidebar(false)} currentSlug={currentSlug} currentModuleId={currentModuleId} />
            </motion.aside>
          </>
        )}
      </AnimatePresence>
